...
STATIC bool S_rck_branchj(pTHX_ RExC_state_t *pRExC_state, rck_params_t *params)
{
    PERL_ARGS_ASSERT_RCK_BRANCHJ;
    ...
    if (params->flags & SCF_DO_STCLASS) {
        ...
        case POSIXA:
            ...
            _invlist_intersection(
                PL_XPosix_ptrs[FLAGS(params->scan)],
...
{
    SSize_t max1 = 0;
    SSize_t min1 = SSize_t_MAX;
    SSize_t num = 0;
    regnode_ssc accum;
    regnode * const startbranch = params->scan;

    PERL_ARGS_ASSERT_RCK_MAKE_TRIE;
...
#define TRIE_TYPE(X) ( ( NOTHING == (X) )                                   \
            ...
            if ( last && trietype ) {
                if ( trietype != NOTHING ) {
                    ...
                    if (
                        (
                            (made == MADE_EXACT_TRIE && startbranch == first)
                            || (params->first_non_open == first)
                        ) && params->depth == 0
                    ) {
                        params->flags |= SCF_TRIE_RESTUDY;
                        if (startbranch == first && params->scan == tail) {
        ...
        regnode * const node, SSize_t mincount, SSize_t maxcount, I32 next_is_eval)
{
   ...
   if ((mincount > 1) || (maxcount > 1 && maxcount != REG_INFTY))
    ...
    if (!(params->flags & SCF_TRIE_DOING_RESTUDY)
        && (params->flags & SCF_DO_SUBSTR)  /* #131868 */
        ...
        && maxcount <= REG_INFTY/3) /* Complement check for big count */
    {
        ...
        Perl_ck_warner(aTHX_ packWARN(WARN_REGEXP),
            ...
            "in regex m/%" UTF8f "/",
             UTF8fARG(UTF, RExC_precomp_end - RExC_precomp, RExC_precomp));
    ...
    if (OP(node) == CURLYX && params->data
        ...
        if (OP(nxt) != CLOSE)
            goto nogo;
        if (RExC_open_parens) {
            RExC_open_parens[ARG(nxt1)] = node; /*open->CURLYM*/
...
STATIC bool S_study_chunk_one_node(pTHX_ RExC_state_t *pRExC_state, rck_params_t *params)
{
    ...
    } else if (OP(params->scan) == BRANCH) {
        return rck_branch(pRExC_state, params);
    } else if (OP(params->scan) == BRANCHJ) {
    ...
    if (! UTF) {
        ...
        if (is_MULTI_CHAR_FOLD_utf8_safe(s, e)) {
                                ...
                                (char *) s, foldlen, FALSE)))
            {
                ...
                bool must_match = (OP(node) == EXACTFA
                        || OP(node) == EXACTFA_NO_TRIE);
                IV k;
                ...
                for (k = 0; k <= av_tindex_skip_len_mg(list); k++) {
                    ...
                    c = SvUV(*c_p);
                    if (must_match && isASCII(c) != isASCII(uc))
                        continue;
